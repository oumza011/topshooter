...
var _base_shell_color := Color(0.86, 0.84, 0.76)
...
func _animate_robot(delta: float, move_amount: float) -> void:
	...
	var stride := move_amount
	var forward_lean := stride * -3.0 - shoot * 5.5
	var lateral_weight := step * 2.8 * stride
	_model_root.position = Vector3(0.0, idle_breath + bob * 0.07 - shoot * 0.035, 0.0)
	_model_root.rotation_degrees = Vector3(forward_lean, 0.0, lateral_weight)

	if is_instance_valid(_head_shell):
		_pose(_head_shell, Vector3(sin(_idle_time * 1.5) * 1.5 - forward_lean * 0.25, sin(_idle_time * 0.9) * 4.0, -step * stride * 1.6 - lateral_weight * 0.25))
	if is_instance_valid(_upper_chest):
		_pose(_upper_chest, Vector3(-shoot * 2.0, 0.0, -step * stride * 1.8))

	if is_instance_valid(_left_forearm):
		_pose(_left_forearm, Vector3(counter_step * 18.0 * stride, 0.0, -8.0 - shoot * 12.0))
	if is_instance_valid(_right_forearm):
		_pose(_right_forearm, Vector3(step * 9.0 * stride - shoot * 28.0, 0.0, 8.0 + shoot * 5.0), Vector3(0.0, 0.0, -shoot * 0.08))
	if is_instance_valid(_right_blaster):
		_pose(_right_blaster, Vector3(shoot * -6.0, 0.0, 0.0), Vector3(0.0, 0.0, -shoot * 0.16))
	...
	if is_instance_valid(_left_thigh):
		_pose(_left_thigh, Vector3(step * 22.0 * stride, 0.0, -3.0 * stride))
	if is_instance_valid(_right_thigh):
		_pose(_right_thigh, Vector3(counter_step * 22.0 * stride, 0.0, 3.0 * stride))
	if is_instance_valid(_left_foot):
		_pose(_left_foot, Vector3(-step * 10.0 * stride, 0.0, 0.0), Vector3(0.0, maxf(step, 0.0) * 0.055 * stride, 0.0))
	if is_instance_valid(_right_foot):
		_pose(_right_foot, Vector3(-counter_step * 10.0 * stride, 0.0, 0.0), Vector3(0.0, maxf(counter_step, 0.0) * 0.055 * stride, 0.0))

	if is_instance_valid(_antenna_tip):
		_pose(_antenna_tip, Vector3.ZERO, Vector3(0.0, sin(_idle_time * 5.0) * 0.025, 0.0))
	...
	if _celebrating and hp > 0:
		...
		if is_instance_valid(_left_forearm):
			_pose(_left_forearm, Vector3(-45.0 + cheer * 12.0, 0.0, -22.0))
		if is_instance_valid(_right_forearm):
			_pose(_right_forearm, Vector3(-42.0 + cheer * 10.0, 0.0, 22.0))
		if is_instance_valid(_visor_glow):
...
func _build_robot() -> void:
	var shell := _mat(_base_shell_color)
	var armor := _mat(Color(0.32, 0.38, 0.39))
	var dark := _mat(Color(0.035, 0.045, 0.055))
	var rubber := _mat(Color(0.07, 0.08, 0.085))
	var trim := _mat(Color(0.95, 0.93, 0.86))
	var warm_shadow := _mat(Color(0.52, 0.53, 0.49))
	...
	if _load_model_scene("res://art/models/robot_ai.glb"):
	...
	_add_ellipsoid("LeftPectoralRoundedPlate", Vector3(-0.32, 1.2, -0.24), Vector3(0.31, 0.34, 0.12), trim)
	_add_ellipsoid("RightPectoralRoundedPlate", Vector3(0.32, 1.2, -0.24), Vector3(0.31, 0.34, 0.12), trim)
	...
	_add_cylinder("CoreAmberInnerRing", Vector3(0.0, 1.16, -0.335), 0.115, 0.025, amber, Vector3(90.0, 0.0, 0.0))
	...
	_add_ellipsoid("HeadTopDome", Vector3(0.0, 1.88, 0.03), Vector3(0.56, 0.22, 0.46), trim)
	_add_ellipsoid("LeftCheekArmor", Vector3(-0.34, 1.62, -0.2), Vector3(0.2, 0.22, 0.18), warm_shadow)
	_add_ellipsoid("RightCheekArmor", Vector3(0.34, 1.62, -0.2), Vector3(0.2, 0.22, 0.18), warm_shadow)
	_add_capsule("HelmetBrow", Vector3(0.0, 1.84, -0.08), 0.08, 0.8, trim, Vector3(0.0, 0.0, 90.0))
	_add_capsule("LeftHelmetEarFin", Vector3(-0.5, 1.82, 0.02), 0.075, 0.36, trim, Vector3(0.0, 0.0, -22.0))
	_add_capsule("RightHelmetEarFin", Vector3(0.5, 1.82, 0.02), 0.075, 0.36, trim, Vector3(0.0, 0.0, 22.0))
	...
	_add_ellipsoid("LeftShoulderIvoryCap", Vector3(-0.76, 1.22, -0.02), Vector3(0.25, 0.22, 0.3), shell, Vector3(0.0, 0.0, 8.0))
	_add_ellipsoid("RightShoulderIvoryCap", Vector3(0.76, 1.22, -0.02), Vector3(0.25, 0.22, 0.3), shell, Vector3(0.0, 0.0, -8.0))
	...
	_add_capsule("LeftForearmDarkRail", Vector3(-0.82, 0.79, -0.17), 0.035, 0.48, dark)
	_add_capsule("RightForearmDarkRail", Vector3(0.82, 0.79, -0.17), 0.035, 0.48, dark)
	...
	_add_sphere("BackpackAmberStatusLeft", Vector3(-0.18, 1.38, 0.57), 0.045, amber)
	_add_sphere("BackpackAmberStatusRight", Vector3(0.18, 1.38, 0.57), 0.045, amber)
	...
	_add_capsule("LeftThighSideRail", Vector3(-0.38, 0.32, -0.05), 0.035, 0.34, armor, Vector3(0.0, 0.0, 8.0))
	_add_capsule("RightThighSideRail", Vector3(0.38, 0.32, -0.05), 0.035, 0.34, armor, Vector3(0.0, 0.0, -8.0))
...
func _cache_animation_parts() -> void:
	_head_shell = _find_mesh("HeadShell")
	_upper_chest = _find_mesh("UpperChestArmor")
	_left_forearm = _find_mesh("LeftForearmArmor")
	_right_forearm = _find_mesh("RightForearmArmor")
	_right_blaster = _find_mesh("RightBlasterBarrel")
	_muzzle_glow = _find_mesh("RightBlasterMuzzleGlow")
	_left_thigh = _find_mesh("LeftThighPlate")
	_right_thigh = _find_mesh("RightThighPlate")
	_left_foot = _find_mesh("LeftFoot")
	_right_foot = _find_mesh("RightFoot")
	_antenna_tip = _find_mesh("AntennaTip")
	_visor_glow = _find_mesh("VisorGlow")
	for part in [_head_shell, _upper_chest, _left_forearm, _right_forearm, _right_blaster, _muzzle_glow, _left_thigh, _right_thigh, _left_foot, _right_foot, _antenna_tip, _visor_glow]:
		_remember_base(part)
	if is_instance_valid(_muzzle_glow):
